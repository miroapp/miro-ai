...
function validateMarketplacePluginPaths(
  ...
  for (const plugin of plugins) {
    ...
    if (
      ...
      pathValue !== `./codex-plugins/${name}`
    ) {
      errors.push(
        `plugin "${name}" must use source.path "./codex-plugins/${name}" (found "${pathValue}")`
      );
    }
  }
...
export async function validateCodexPlugins(
  ...
  const pluginFiles = await fg("codex-plugins/*/.codex-plugin/plugin.json", {
    cwd: root,
  });
