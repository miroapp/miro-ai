...
└── skills/                 # 3 skills, byte-identical to the source plugin
```
...
Gemini CLI clones the repo into `~/.gemini/extensions/miro/`, reads `gemini-extension.json` to register the Miro MCP server, and discovers the 3 skills under `skills/` as Extension Skills. Restart Gemini CLI and authenticate when prompted.
