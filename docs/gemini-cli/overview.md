...
└── skills/                 # 8 skills, byte-identical to the source plugin
```
...
Gemini CLI clones the repo into `~/.gemini/extensions/miro/`, reads `gemini-extension.json` to register the Miro MCP server, and discovers the 8 skills under `skills/` as Extension Skills. Restart Gemini CLI and authenticate when prompted.
